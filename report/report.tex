...
Για την πρόβλεψη της τιμής ασφάλισης χρησιμοποιήθηκε το σύνολο δεδομένων \\"Motor\_vehicle\_insurance\_data.csv" \href{https://www.openicpsr.org/openicpsr/project/193182/version/V1/view;jsessionid=2A18A6C567D7ABED7D35F05219396A48}{[Πηγή]}
το οποίο βρίσκεται στα παραδοτέα αρχεία. Η διαδικασία της εργασίας ξεκινά με την κατάλληλη προ-επεξεργασία 
των δεδομένων, κατά την οποία πραγματοποιήθηκε ανάλυση των δεδομένων για τον προσδιορισμό των κριτηρίων διαχωρισμού τους αλλά και του βαθμού επίδρασης κάθε χαρακτηριστικού (feature) 
...
πόσα χρόνια οδηγεί ο πελάτης, 'Year\_on\_road' που προσδιορίζει πόσα χρόνια κυκλοφορεί το κάθε όχημα υπό την κατοχή συγκεκριμένου πελάτη και 
...
που δημιουργεί όσες στήλες όσες και οι μοναδικές τιμές κάθε κατηγορίας. Για κάθε στήλη 
βάζει 1 αν ανήκει και 0 αν δεν ανήκει στην κάθε κατηγορία.
...
\begin{figure}
    ...
    \end{center}
    \caption{Αριθμός εγγραφών ανά κατηγορία οχήματος} 
    \label{fig:Type_risk}   
\end{figure}
...
\begin{observation}
    ...
    και ότι υπάρχει μεγάλη διαφορά των Premium σε κάθε κατηγορία οχήματος.[Εικ. \ref{fig:subfigures4}]
\end{observation}
...
να εκπαιδεύσουμε τα μοντέλα για κάθε τύπο οχήματος ξεχωριστά. Για την εκπαίδευση των μοντέλων 
έχει χρησιμοποιηθεί το 80\% των δεδομένων και για την αξιολόγηση το 20 \%.

\subsection{XGBoost}
Το πρώτο μοντέλο πρόβλεψης που χρησιμοποιήσαμε ήταν το XGBoost. Είναι ένα μοντέλο μηχανικής 
μάθησης που βασίζεται στη μέθοδο gradient boosting. Πολλαπλά δέντρα 
αποφάσεων κατασκευάζονται διαδοχικά με σκοπό να διορθώσουν τα σφάλματα 
των προηγούμενων. Παρακάτω αναφέρονται οι παράμετροι που επιλέχθηκαν για το συγκεκριμένο μοντέλο, 
ενώ στις παρενθέσεις βρίσκεται το συνηθισμένο εύρος τιμών, δηλαδή το εύρος που 
έγιναν οι δοκιμές για να πετύχουμε το βέλτιστο αποτέλεσμα.\\
...
Το δεύτερο μοντέλο πρόβλεψης που χρησιμοποιήσαμε ήταν το RandomForest. Είναι μοντέλο 
μηχανικής μάθησης που αποτελείται από πολλαπλά δέντρα απόφασης και 
συνδυάζει τα αποτελέσματά τους. Παρακάτω αναφέρονται οι παράμετροι που επιλέχθηκαν για το συγκεκριμένο μοντέλο, 
ενώ στις παρενθέσεις βρίσκεται το συνηθισμένο εύρος τιμών, δηλαδή το εύρος που έγιναν 
οι δοκιμές για να πετύχουμε το βέλτιστο αποτέλεσμα.\\
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|} % 4 columns
        ...
        \hline
    \end{tabular}
    \caption{Αξιολόγηση XGBoost ανά κατηγορία που εκπαιδεύτηκε στο σύνολο των δεδομένων}
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|} % 4 columns
        ...
        \hline
    \end{tabular}
    \caption{Αξιολόγηση Random Forest ανά κατηγορία που εκπαιδεύτηκε στο σύνολο των δεδομένων }
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|} % 4 columns
        ...
        \hline
    \end{tabular}
    \caption{Αξιολόγηση XGBoost που εκπαιδεύτηκε ξεχωριστά σε κάθε κατηγορία}
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|} % 4 columns
        ...
        \hline
    \end{tabular}
    \caption{Αξιολόγηση Random Forest που εκπαιδεύτηκε ξεχωριστά σε κάθε κατηγορία}
...
\begin{figure}
  ...
  \vfill
       ...
       \label{fig:subxgb3}
        ...
        \label{fig:subxgb4}
         \end{subfigure}
  \caption{Σημασία χαρακτηριστικών σε μοντέλο XGBoost που εκπαιδεύτηκε ξεχωριστά ανα κατηγορία}
  ...
  \vfill
       ...
       \label{fig:subrandom3}
        ...
        \label{fig:subrandom4}
         \end{subfigure}
  \caption{Σημασία χαρακτηριστικών σε μοντέλο Random Forest που εκπαιδεύτηκε ξεχωριστά ανα κατηγορία}
...
Απο την παραπάνω ανάλυση προκύπτει ότι το καλύτερο μοντέλο είναι το \textbf{XGBoost} 
...
ενώ λιγότερες απο 3 \% των προβλέψεων έχουν απόκλιση πάνω απο 80 \% της πραγματικής τιμής.
Αυτή η απόκλιση μπορεί να οφείλεται σε έλλειψη πληροφορίας απο το δεδομένα μας αφού ενδέχεται
η ασφαλιστική εταιρεία να μην δημοσίευσε όλες τις παραμέτρους που χρησιμοποιεί για να 
...
Παρατηρούμε επίσης ότι μεγάλη σημασία έχει και ο τρόπος που αγοράστηκε η ασφάλεια αφού 
όσοι αγόρασαν την ασφάλεια κατευθείαν από την εταιρεία έχουν καλύτερη τιμή απο όσους την 
...
κοστίζει η επισκευή του. Παρατηρούμε ότι αυτό είναι σημαντικότερος παράγοντας απόφασης στις
κατηγορίες με μεγαλύτερο εύρος αξίας των οχημάτων κατά την αγορά τους.
Για τις μοτοσυκλέτες είναι λογικό 
τα πιο σημαντικά χαρακτηριστικά να είναι ο κυβισμός τους και η ηλικία του οδηγού τους αφού
οι νέοι οδηγοί και οι μηχανές με υψηλό κυβισμό προκαλούν πιο πολλά ατυχήματα.
...
Για να μπορέσει να λειτουργήσει η εφαρμογή χρειάζεται αρχικά η εγκατάσταση της 
\href{https://www.python.org/downloads/release/python-3123/}{Python 3.12.2} και μετά η
...
οποίο θα εγκατασταθεί το περιβάλλον και ό,τι άλλο χρειαστούμε
για την εφαρμογή (π.χ. ονομάζεται app).Ανοίγουμε ένα τερμάτικό στα Windows 
και πλοηγούμαστε εκεί. Έπειτα, με την εντολή: 
\begin{verbatim}
python3 -m venv virt
\end{verbatim} 
...
\end{verbatim} το ενεργοποιούμε. Με την παρακάτω εντολή αναβαθμίζουμε και το pip:
\begin{verbatim}
python.exe -m pip install --upgrade pip
\end{verbatim}

\subsection{Εγκατάσταση των απαραίτητων βιβλιοθηκών για την εφαρμογή}
...
με τα αρχεία ui και xgboostModels και χρησιμοποιούμε την εντολή: 
\begin{verbatim}
python ui\front.py
\end{verbatim}
Για τον τρόπο λειτουργίας της εφαρμογής ακολουθούμε τις οδηγίες χρήσης στην επόμενη ενότητα.
...
Mετά την εγκατάσταση μπορείτε να τρέξετε οποιοδήποτε από τα script και να δείτε τα διαγράμματα
...
Στα υπόλοιπα πεδία ο χρήστης θα πρέπει να πληκτρολογήσει έναν αριθμό σύμφωνα με τις υποδείξεις του κάθε πεδίου (ενδεικτικά [Εικ. \ref{fig:reg}]). Σημειώνεται ότι το βάρος του οχήματος πρέπει να συμπληρωθεί σε κιλά, το μήκος σε μέτρα και η αξία στην οποία αγοράστηκε σε €.
...
κατά την επιλογή τους εμφανίζεται μενού επιλογών (ενδεικτικά [Εικ. \ref{fig:pay}]), ενώ τα υπόλοιπα πρέπει να συμπληρωθούν με πληκτρολόγηση σύμφωνα με τις υποδείξεις των πεδίων. Σημειώνεται ότι στα πεδία προς πληκτρολόγηση πρέπει να δοθούν θετικοί αριθμοί.
...
\begin{figure}
    ...
    \end{center}
    \caption{Οθόνη συμπλήρωσης στοιχείων συμβολαίου}  
    \label{fig:contract}  
\end{figure}
